...
- Feature: RateLimit CRDs now support specifying an `action` for each limit. Valid values include "Enforce" and "LogOnly" and are case insensitive. LogOnly may be used to implement dry run rules that do not actually enforce limits.
- Feature: RateLimit CRDs now support specifying a symbolic `name` for each limit. This name can be referenced in the access logs to know which limit, if any, applied to a request.
