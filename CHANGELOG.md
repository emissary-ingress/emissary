...
- Change: A new field is added to the `Docs` field in the `Mapping` resource: `timeout_ms` is an optional integer that, when present, configures the number of milliseconds the Devportal HTTP client will wait to access the docs endpoint of the service mapping. 
...
- Change: Consul certificate-rotation logging now includes the fingerprints and validity timestamps of certificates
  being rotated.
- Change: Devportal leverages the `timeout_ms` field in the `docs` field of the `Mapping` resource to configure the HTTP client it uses to fetch API specifications.
