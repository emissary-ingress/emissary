...
- Feature: The healthcheck server's bind address, bind port and IP family can now be configured
  ...
  to. AMBASSADOR_HEALTHCHECK_IP_FAMILY - The IP family to use for the healthcheck server. The main
  advantage of this is that it allows the healthcheck server to be configured to use IPv6-only k8s
  environments.
