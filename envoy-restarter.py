...
def fork_and_exec():
  ...
  if child_pid == 0:
    # Child process
    os.execl(envoy_command, envoy_command)
  else:
    # Parent process
...
def main():
  ...
  print("envoy-restarter (PID {}) starting".format(pid))
  print("    pidfile: {}".format(pid_path))
  print("    command: {}".format(envoy_command))

  open(pid_path, "w").write("{}\n".format(pid))
