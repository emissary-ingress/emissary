...
module_version() {
    ...
    if [[ ${GIT_DESCRIPTION} =~ - ]]; then
        ...
        if [[ ${GIT_REST} =~ ^[a-z] ]] && [[ ${GIT_REST} =~ - ]]; then
            # git describe isn't exactly at an rc tag so let's filter those out
            # when getting the git description
        ...
        # at an "-rc" tag or the like, and there should not be another "-" in it.
