...
'
    ...
    if ! docker image inspect "$name1" &>/dev/null; then # skip building if the "$name1" already exists
    ...
    if ! docker image inspect "$name2" &>/dev/null; then # skip building if the "$name2" already exists
