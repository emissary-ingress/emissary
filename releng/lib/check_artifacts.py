    ...
    def do_check_docker(checker: Checker, name: str) -> None:
        with checker.check(name=f'Docker image: {name}', clear_on_success=False) as check:
            ...
            for registry in registries:
                tags = [rc_ver]
                if include_latest and not is_private:
                    tags = tags + [f'{ga_ver}-rc-latest']
                if ga:
                    tags = [ga_ver] + tags
                for tag in tags:
                ...
                with do_check_s3(checker, name='ambassador/app.json', bucket='scout-datawire-io',
                    ...
                    if body is not None:
                        ...
                        if is_private:
                            ...
                            assert_eq(subcheck.result, ga_ver)
        do_check_binary(checker, 'edgectl', txt=True, private=False)
        do_check_binary(checker, 'aes-plugin-runner', txt=False, private=False)
        do_check_binary(checker, 'apictl-key', txt=True, private=True)
