...
func TestHTTPExternalModify(t *testing.T) {
	t.Parallel()
	assert := &testutil.Assert{T: t}
	res, body := doRequest(t, "https://ambassador.ambassador.svc.cluster.local/external-http/headers") //nolint:bodyclose
	// HTTP/1.1 200 OK
...
func TestHTTPExternalIntercept(t *testing.T) {
	t.Parallel()
	assert := &testutil.Assert{T: t}
	res, body := doRequest(t, "https://ambassador.ambassador.svc.cluster.local/external-http/ip") //nolint:bodyclose
...
func TestHTTPExternalInterceptWithRedirect(t *testing.T) {
	t.Parallel()
	assert := &testutil.Assert{T: t}
	res, body := doRequest(t, "https://ambassador.ambassador.svc.cluster.local/external-http/redirect") //nolint:bodyclose
	// HTTP/1.1 302 Found
...
func TestGRPCExternalModify(t *testing.T) {
	t.Parallel()
	assert := &testutil.Assert{T: t}
	res, body := doRequest(t, "https://ambassador.ambassador.svc.cluster.local/external-grpc/headers") //nolint:bodyclose
	// HTTP/1.1 200 OK
	// access-control-allow-credentials: true
	// access-control-allow-origin: *
	// content-type: application/json
...
func TestGRPCExternalIntercept(t *testing.T) {
	t.Parallel()
	assert := &testutil.Assert{T: t}
	res, body := doRequest(t, "https://ambassador.ambassador.svc.cluster.local/external-grpc/ip") //nolint:bodyclose
	// HTTP/1.1 200 OK
	// content-length: 22
	// content-type: application/json
