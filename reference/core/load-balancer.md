...
Load balancing configuration can be set for all Ambassador Edge Stack mappings in the [`ambassador`](../ambassador) Module, or set per [mapping](../../mappings#configuring-mappings). If nothing is set, simple round robin balancing is used via Kubernetes services.
