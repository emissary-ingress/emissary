...
| `idle_timeout` | Controls how Envoy configures the tcp idle timeout on the http listener. Default is no timeout (TCP connection may remain idle indefinitely). NOTE: only s (seconds) is supported for the duration suffix | `idle_timeout: 300s` |
