...
func processFilterSpec(filter k8s.Resource, cfg types.Config, coreClient *k8sClientCoreV1.CoreV1Client) FilterInfo {
...
func (c *Controller) Watch(ctx context.Context, kubeinfo *k8s.KubeInfo) error {
	...
	w.Watch("filters", func(w *k8s.Watcher) {
		...
		for _, mw := range w.List("filters") {
			filterInfo := processFilterSpec(mw, c.Config, coreClient)
