...
func ReconcileSecrets(s *types.AmbassadorInputs) {
	...
	refs := map[types.SecretRef]bool{}

	// ...and, uh, this "action" function is really just a closure to avoid
	// needing to pass "refs" to findtypes.SecretRefs. Shrug. Arguably more
	// complex than needed, but meh.
	action := func(ref types.SecretRef) {
		refs[ref] = true
	}
...
func findSecretRefs(resource kates.Object, secretNamespacing bool, action func(types.SecretRef)) {
...
func secretRef(namespace, name string, secretNamespacing bool, action func(types.SecretRef)) {
