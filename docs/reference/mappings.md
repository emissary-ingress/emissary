...
| `idle_timeout_ms`         | the timeout, in milliseconds, after which connections through this `Mapping` will be terminated if no traffic is seen -- if not present, no timeout is applied. |
