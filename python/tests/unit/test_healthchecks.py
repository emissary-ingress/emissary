        ...
        if expectedChecks is None:
            ...
            for i in range(0, len(hc)):
                ...
                if "grpc_health_check" in expected:
                    try:
                        check_grpc_healthcheck(expected["grpc_health_check"], actual["grpc_health_check"], testName)
                    except KeyError:
                        assert True == False, "Failed healthcheck test {}".format(testName)
                if "http_health_check" in expected:
                    try:
                        check_http_healthcheck(expected["http_health_check"], actual["http_health_check"], testName)
                    except KeyError:
                        assert True == False, "Failed healthcheck test {}".format(testName)
...
def check_healthcheck_defaults(expected, actual, testName):
    ...
    if "healthy_threshold" in expected:
        assert actual["healthy_threshold"] == expected["healthy_threshold"], "Failed healthcheck test {}".format(testName)
    else:
        assert actual["healthy_threshold"] == 1, "Failed healthcheck test {}".format(testName)

    if "interval" in expected:
        assert actual["interval"] == expected["interval"], "Failed healthcheck test {}".format(testName)
    else:
        assert actual["interval"] == "5s", "Failed healthcheck test {}".format(testName)

    if "timeout" in expected:
        assert actual["timeout"] == expected["timeout"], "Failed healthcheck test {}".format(testName)
    else:
        assert actual["timeout"] == "3s", "Failed healthcheck test {}".format(testName)

    if "unhealthy_threshold" in expected:
        assert actual["unhealthy_threshold"] == expected["unhealthy_threshold"], "Failed healthcheck test {}".format(testName)
    else:
...
def check_grpc_healthcheck(expected, actual, testName):
    if expected is not None:
        ...
        assert actual["service_name"] == expected["service_name"], "Failed healthcheck test {}".format(testName)

        if "authority" in expected:
            assert actual["authority"] == expected["authority"], "Failed healthcheck test {}".format(testName)
...
def check_http_healthcheck(expected, actual, testName):
    if expected is not None:
        ...
        if "request_headers_to_remove" in expected:
            assert actual["request_headers_to_remove"] == expected["request_headers_to_remove"], "Failed healthcheck test {}".format(testName)

        if "request_headers_to_add" in expected:
            assert actual["request_headers_to_add"] == expected["request_headers_to_add"], "Failed healthcheck test {}".format(testName)

        if "expected_statuses" in expected:
            assert actual["expected_statuses"] == expected["expected_statuses"], "Failed healthcheck test {}".format(testName)
