...
def test_proper_case_true():
    ...
    for v in SUPPORTED_ENVOY_VERSIONS:
        _test_listener_http_protocol_options(
            yaml, expectations={"header_key_format": {"proper_case_words": {}}}, envoy_version=v
        )
...
def test_proper_case_and_enable_http_10():
    ...
    for v in SUPPORTED_ENVOY_VERSIONS:
        _test_listener_http_protocol_options(
            yaml,
            expectations={"accept_http_10": True, "header_key_format": {"proper_case_words": {}}},
            envoy_version=v,
        )
...
def test_allow_chunked_length_false():
    ...
    _test_listener_http_protocol_options(
        yaml, expectations={"allow_chunked_length": False}, envoy_version="V3"
    )
...
def test_allow_chunked_length_true():
    ...
    _test_listener_http_protocol_options(
        yaml, expectations={"allow_chunked_length": True}, envoy_version="V3"
    )
