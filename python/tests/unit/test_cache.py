...
class Builder:
    def __init__(self, logger: logging.Logger, yaml_file: str, enable_cache=True) -> None:
        ...
        self.secret_handler = NullSecretHandler(
            logger, "/tmp/secrets/src", "/tmp/secrets/cache", "0"
        )
...
class MadnessOp:
    ...
    def __init__(
        self, name: str, op: str, mapping: MadnessMapping, verifiers: List[MadnessVerifier]
    ) -> None:
        self.name = name
        self.op = op
    ...
    def exec(self, builder1: Builder, builder2: Builder, dumpfile: Optional[str] = None) -> bool:
        ...
        if dumpfile:
            json.dump(
                b1[0].as_dict(), open(f"/tmp/{dumpfile}-1.json", "w"), indent=2, sort_keys=True
            )
            json.dump(
                b2[0].as_dict(), open(f"/tmp/{dumpfile}-2.json", "w"), indent=2, sort_keys=True
            )
