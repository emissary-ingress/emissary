...
"""
    ...
    def queries(self):
        yield Query(
            self.url(self.name + "-normal/"), headers={"Requested-Backend-Delay": "0"}, expected=200
        )
        yield Query(
            self.url(self.name + "-normal/"), headers={"Requested-Status": "500"}, expected=500
        )
        yield Query(
            self.url(self.name + "-retry/"),
            headers={"Requested-Status": "500", "Requested-Backend-Delay": "2000"},
            expected=504,
        )
        yield Query(
            self.url(self.name + "-normal/"),
            headers={"Requested-Status": "409", "Requested-Backend-Delay": "2000"},
            expected=504,
        )
