...
""")
    ...
    def queries(self):
        ...
        yield Query(self.url("config_dump"), expected=400, headers={'t_underscore':'foo'})

    def check(self):
        expected_val = 'REJECT_REQUEST'
        actual_val = ''
        ...
        for config_obj in body.get('configs'):
          if config_obj.get('@type') == 'type.googleapis.com/envoy.admin.v3.ListenersConfigDump':
            ...
            found_headers_with_underscores_action = False
                ...
                for filter in filters.get('filters'):
                  if filter.get('name') == 'envoy.filters.network.http_connection_manager':
                    ...
                    if common_http_protocol_options:
                      actual_val = common_http_protocol_options.get('headers_with_underscores_action', '')
                      if actual_val != '':
                        if actual_val == expected_val:
                          found_headers_with_underscores_action = True
                      else:
                        assert False, "Expected to find common_http_protocol_options.headers_with_underscores_action property on listener"
                    else:
                      assert False, "Expected to find common_http_protocol_options property on listener"
            assert found_headers_with_underscores_action, "Expected common_http_protocol_options.headers_with_underscores_action = {}, Got common_http_protocol_options.headers_with_underscores_action = {}".format(expected_val, actual_val)
