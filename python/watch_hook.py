...
class SecretRecorder(SecretHandler):
    ...
    def load_secret(
        ...
        self.logger.debug(
            "SecretRecorder (%s %s): load secret %s in namespace %s"
    ...
    def record_secret(self, secret_name: str, namespace: str) -> Optional[SecretInfo]:
        ...
        if secret_key not in self.needed:
            self.needed[secret_key] = SecretInfo(
                secret_name, namespace, "needed-secret", "-crt-", "-key-", decode_b64=False
            )
    ...
    def still_needed(self, resource: "IRResource", secret_name: str, namespace: str) -> None:
        self.logger.debug(
            "SecretRecorder (%s %s): secret %s in namespace %s is still needed"
            % (resource.kind, resource.name, secret_name, namespace)
        )
    ...
    def cache_secret(self, resource: "IRResource", secret_info: SecretInfo):
        self.logger.debug(
            "SecretRecorder (%s %s): skipping cache step for secret %s in namespace %s"
...
class FakeIR(IR):
    def __init__(self, aconf: Config, logger=None) -> None:
        ...
        super().__init__(
            ...
            logger=logger,
            watch_only=True,
            secret_handler=self.secret_recorder,
...
class WatchHook:
    ...
    def load_yaml(self, yaml_stream):
        ...
        cloud_connect_token_resource_name = os.getenv(
            ENV_CLOUD_CONNECT_TOKEN_RESOURCE_NAME, DEFAULT_CLOUD_CONNECT_TOKEN_RESOURCE_NAME
        )
        cloud_connect_token_resource_namespace = os.getenv(
            ENV_CLOUD_CONNECT_TOKEN_RESOURCE_NAMESPACE, Config.ambassador_namespace
        )
        self.logger.debug(
            f"cloud-connect-token: need configmap/secret {cloud_connect_token_resource_name}.{cloud_connect_token_resource_namespace}"
        )
        self.add_kube_watch(
            f"ConfigMap {cloud_connect_token_resource_name}",
            "configmap",
            namespace=cloud_connect_token_resource_namespace,
            field_selector=f"metadata.name={cloud_connect_token_resource_name}",
        )
        self.add_kube_watch(
            f"Secret {cloud_connect_token_resource_name}",
            "secret",
            namespace=cloud_connect_token_resource_namespace,
            field_selector=f"metadata.name={cloud_connect_token_resource_name}",
        )
        ...
        for host in self.fake.get_hosts():
            ...
            for wanted_kind in ["service", "secret"]:
                self.add_kube_watch(
                    f"Host {host.name}", wanted_kind, host.namespace, label_selector=label_selector
                )
            ...
            if res_name:
                ...
                if resolver:
                    ...
                    if resolver.kind == "ConsulResolver":
                        ...
                        self.logger.debug(
                            f"...kube endpoints: svc {svc.hostname} -> host {hostname} namespace {namespace}"
                        )
        ...
        if self.fake.edge_stack_allowed:
            ...
            self.add_kube_watch(
                "Fallback TLSContext", "TLSContext", namespace=Config.ambassador_namespace
            )
        ...
        ambassador_knative_requested = (
            os.environ.get("AMBASSADOR_KNATIVE_SUPPORT", "-unset-").lower() == "true"
        )
        ...
        if ambassador_knative_requested:
            ...
            if os.path.exists(os.path.join(ambassador_basedir, ".knative_ingress_ok")):
                ...
                self.add_kube_watch(
                    "Knative ingresses", "ingresses.networking.internal.knative.dev", None
                )
