...
func main() {
	...
	argparser := &cobra.Command{
		Use:   "git-godescribe [flags] [COMMITISH]",
...
func Describe(ctx context.Context, commitish, dirPrefix string) (string, error) {
	...
	commit, err := statLocal(ctx, commitish)
	if err != nil {
		return "", err
	}

	parentTag, err := mostRecentTag(ctx, commit, dirPrefix)
	if err != nil {
		return "", err
	}

	goVersionStr := module.PseudoVersion(
		semver.Major(parentTag),
		parentTag,
		commit.Time,
		ShortenSHA1(commit.Hash))
