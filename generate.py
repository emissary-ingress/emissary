...
def has_entrypoints(scriptbody: str) -> bool:
    main = re.findall('\nif\s*__name__\s*==\s*["\']__main__["\']\s*:', scriptbody)
    unindented = re.findall('\n\S+\([\S+]?\)', scriptbody)
    return len(main+unindented) > 0
...
def rules_for_script(workspacedir: str, inputdir: str, filepath: str) -> List[str]:
    ...
    elif has_entrypoints(filecontent):
        return [f"""py_binary(
    name = {quote(os.path.basename(filepath)[:-len('.py')])},
    srcs = [{quote(os.path.basename(filepath))}],
    deps = {depstr},
