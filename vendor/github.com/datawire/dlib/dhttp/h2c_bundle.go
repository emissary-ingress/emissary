...
//   $ bundle golang.org/x/net/http2/h2c
...
func h2c_NewHandler(h http.Handler, s *http2.Server) http.Handler {
	return &h2c_h2cHandler{
		Handler: h,
		s:       s,
	}
}
...
func (s h2c_h2cHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	// Handle h2c with prior knowledge (RFC 7540 Section 3.4)
	if r.Method == "PRI" && len(r.Header) == 0 && r.URL.Path == "*" && r.Proto == "HTTP/2.0" {
		if h2c_http2VerboseLogs {
			log.Print("h2c: attempting h2c with prior knowledge.")
		}
		conn, err := h2c_initH2CWithPriorKnowledge(w)
		if err != nil {
			if h2c_http2VerboseLogs {
	...
	if conn, err := h2c_h2cUpgrade(w, r); err == nil {
		defer conn.Close()
...
// creates.
func h2c_initH2CWithPriorKnowledge(w http.ResponseWriter) (net.Conn, error) {
...
// the supplied reader.
func h2c_drainClientPreface(r io.Reader) error {
...
func h2c_h2cUpgrade(w http.ResponseWriter, r *http.Request) (net.Conn, error) {
	if !h2c_isH2CUpgrade(r.Header) {
		return nil, errors.New("non-conforming h2c headers")
	}

	// Initial bytes we put into conn to fool http2 server
	initBytes, _, err := h2c_convertH1ReqToH2(r)
	if err != nil {
		return nil, err
	}
	...
	if err := h2c_drainClientPreface(rw); err != nil {
		return nil, err
	}
	...
	c := &h2c_rwConn{
		...
		BufWriter: h2c_newSettingsAckSwallowWriter(rw.Writer),
	}
	return c, nil
}
...
func h2c_convertH1ReqToH2(r *http.Request) (*bytes.Buffer, []http2.Setting, error) {
	h2Bytes := bytes.NewBuffer([]byte((http2.ClientPreface)))
	framer := http2.NewFramer(h2Bytes, nil)
	settings, err := h2c_getH2Settings(r.Header)
	if err != nil {
		return nil, nil, err
	}
	...
	headerBytes, err := h2c_getH2HeaderBytes(r, h2c_getMaxHeaderTableSize(settings))
	if err != nil {
		return nil, nil, err
	}

	maxFrameSize := int(h2c_getMaxFrameSize(settings))
...
func h2c_getMaxFrameSize(settings []http2.Setting) uint32 {
...
func h2c_getMaxHeaderTableSize(settings []http2.Setting) uint32 {
	for _, setting := range settings {
...
type h2c_rwConn struct {
	net.Conn
	io.Reader
	BufWriter h2c_bufWriter
}

// Read forwards reads to the underlying Reader.
func (c *h2c_rwConn) Read(p []byte) (int, error) {
	return c.Reader.Read(p)
}

// Write forwards writes to the underlying bufWriter and immediately flushes.
func (c *h2c_rwConn) Write(p []byte) (int, error) {
...
type h2c_settingsAckSwallowWriter struct {
	Writer     *bufio.Writer
	buf        []byte
	didSwallow bool
}
...
func h2c_newSettingsAckSwallowWriter(w *bufio.Writer) *h2c_settingsAckSwallowWriter {
	return &h2c_settingsAckSwallowWriter{
		Writer:     w,
		buf:        make([]byte, 0),
		didSwallow: false,
	}
...
func (w *h2c_settingsAckSwallowWriter) Write(p []byte) (int, error) {
	if !w.didSwallow {
...
func h2c_getH2Settings(h http.Header) ([]http2.Setting, error) {
	...
	settings, err := h2c_decodeSettings(vals[0])
	if err != nil {
...
func h2c_decodeSettings(headerVal string) ([]http2.Setting, error) {
...
func h2c_getH2HeaderBytes(r *http.Request, maxHeaderTableSize uint32) ([]byte, error) {
...
func h2c_isNonH2Header(header string) bool {
	for _, nonH2h := range h2c_nonH2Headers {
		if header == nonH2h {
			return true
		}
	}
